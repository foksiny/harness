...
- 🌐 **Web Control (Chrome via CDP)**:
  ...
  - 15 tools: `browser_launch`, `browser_navigate`, `browser_click`, `browser_type`, `browser_press_key`, `browser_scroll`, `browser_screenshot`, `browser_evaluate`, `browser_get_page_info`, `browser_tab`, `browser_navigation`, `browser_close`, `browser_console`, `browser_network`, `browser_wait`.
  - Page debugging without in-page listeners: `browser_console` reads console messages and uncaught JS errors buffered via CDP (survives navigation/reload, so initial-load errors are visible after the fact), `browser_network` shows failed requests and HTTP 4xx/5xx, and `browser_wait` waits for a selector or visible text instead of fixed sleeps.
